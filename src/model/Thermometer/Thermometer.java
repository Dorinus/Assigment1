...
import model.Radiator.PropretyChangeRadiator;
...
public class Thermometer
{
  ...
  public Thermometer(double t, int distance, String name)
  {
  ...
  public Thermometer(PropretyChangeThermometer thermometer, PropretyChangeRadiator radiator)
  {

    thermometer.addListener(this::reactToThermometer);
    radiator.addListener(this::reactToRadiator);

  }

  private void reactToRadiator(PropertyChangeEvent event)
  {
    p =  (Integer) event.getNewValue();
  }

  public void reactToThermometer(PropertyChangeEvent event)
  {
    t0 = (Double) event.getNewValue();
  ...
  public double getCurrentTemperature()
  {
    ...
    if (p > 0)
    {
      double den = Math.max((tMax * (20 - 5 * p) * (distance + 5)), 0.1);
